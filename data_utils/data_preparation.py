...
class DataPreparator:
    ...
    def _prepare_data_lei_2020(self):
        ...
        df = self._compute_and_append_sorting_metrics(df)

        # add wall information
        df = self._compute_wall_metrics(df)
    ...
    def _compute_and_append_sorting_metrics(self, df):
    ...
    def _compute_wall_metrics(self, df, radius=0.25):
        # Distance to center
        dist_to_origin = np.sqrt(df["x"]**2 + df["y"]**2)

        # Closest point on the wall
        df["x_wall"] = (df["x"] / dist_to_origin) * radius
        df["y_wall"] = (df["y"] / dist_to_origin) * radius

        # Vector fish → wall
        vec_x = df["x_wall"] - df["x"]
        vec_y = df["y_wall"] - df["y"]

        # Normalize to unit vector (repulsion direction)
        norm = np.sqrt(vec_x**2 + vec_y**2)
        df["repulse_x"] = vec_x / norm
        df["repulse_y"] = vec_y / norm

        # Tangent vector at the wall (perpendicular to radius)
        tan_x = -df["y_wall"]
        tan_y = df["x_wall"]
        tan_norm = np.sqrt(tan_x**2 + tan_y**2)
        df["tangent_x"] = tan_x / tan_norm
        df["tangent_y"] = tan_y / tan_norm

        # Distance to wall
        df["dist_to_wall"] = radius - dist_to_origin
